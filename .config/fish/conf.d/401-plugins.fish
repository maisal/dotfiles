...
# set -x FZF_COMPLETE 1
  ...
  set -x FZF_PREVIEW_CMD "if test -d {};eza --no-permissions --no-time --no-user -l --sort type {};else;bat -p --color=always {};end"
  ...
  if type -q $FD
    ...
    set -x FZF_ALT_C_OPTS "--preview 'eza --no-permissions --no-time --no-user -l --sort type {}'"
